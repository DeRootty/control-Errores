            ...
            if(!isset($this->queTablaSQL[0])){
                foreach($ixBDConexion[$ixers_Login["permission"][$userOK][$idsLogines[3]]]["Tablas"] as $idValZ => $valEndZ){
                    ...
                    if((isset($this->queTablaSQL))){
                        ...
                        echo "no se ha establecido una tabla de consulta<br>";
                        exit;
                    }
    ...
    class cabeceraForm{
        ...
        public function __construct($checkFieldLong, $camposFrmTT){
            ...
            foreach($this->iniVal as $idVal => &$valEnd){
                $this->alphanumeric=str_shuffle($this->alphanumeric);
                $this->dasphalmeneric=str_shuffle($this->alphanumeric);
                //Extraemos una cadena de longitud $checkFieldLong
                $this->valorRand=substr($this->dasphalmeneric,$valEnd,$checkFieldLong);
                do{
                    //se extrae la primera coincidencia de la cadena de ocho seleccionada y se verifica que no sea un numero
                    $test=substr($this->valorRand,0,1);
                    if(is_numeric($test)){
                        //Si la coincidencia es numerica, se busca otro valor aleatorio en rango $this->valMax
                        $valEnd=random_int(0,$this->valMax);
                        //Se reintenta la extraccion de una cadena de longitud $checkFieldLong
                        $this->valorRand=substr($this->dasphalmeneric,$valEnd,$checkFieldLong);
                        $repeat=true;
                    }else{
        ...
        function salidaRandomField($tagFieldFrm,$visible,$pathHost){
            ...
            /*
            echo "IdAlta:... ".$this->idAlta."<br>";
            exit;
            */

            $this->cabezaFrm ="<form action=".chr(39).$pathHost."?".trim($salida)."&".trim($this->idAlta)."=nuevoAlta".chr(39)." method=".chr(39)."post".chr(39).">"."\n";
