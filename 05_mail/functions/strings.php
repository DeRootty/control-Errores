...
}else if(file_exists(SM_PATH . $miNombre->iknowiam .'/functions/global.php')){
    require_once(SM_PATH . $miNombre->iknowiam .'/functions/global.php');
}else{
    echo "ERROR de carga de archivo:" . SM_PATH . $miNombre->iknowiam .'/functions/global.php';
    exit;
}
...
function sqm_baseuri(){
    ...
    $repl = array('', '', '');
    //(patron de busqueda, cadenas a reemplazar, objetivo de busquedas a sustituir)
    if(empty($PHP_SELF)){
        throw new Exception('Peticion desde la raiz de dominio');
    }else{
