...
iniciando:    
    ...
    //$testingConn = new test\testVal($dataConn);
    $setResponse = json_decode($testingConn->pruebas($dataConn), true);
    $retorno = $modoRtm->getDatEngEntorno($setResponse);
    try{
    ...
    foreach($retorno as $idVal => $valEnd){
        ...
        $base = implode("/", $defrag);
        try{
            if($localServer != $_SERVER['SERVER_NAME']){
                $test = json_encode(array(
                    'Estado' => 'Error',
                    'Leyenda' => 'conexion fallida en la recuperacion de datos nivel basico. Intento en el ID -> '.$idVal,
                    'Funcion' => 'require_flow',
                    'Clase' => 'null_instance',
                    'Especial' => 'no_traits',
    ...
    foreach($launchDataConn as $idVal => $valEnd){
        ...
        $pasaPalabra = json_encode($valEnd);
        $passTo = $testingConn->pruebas($pasaPalabra);
