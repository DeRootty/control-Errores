<?php declare(strict_types=1);
    ...
    namespace practicasAPP\dinamica;
    use Exception;
    $modoRtm->registroMod("Hola mundo en Entrada en dinamica ". __FILE__ . " => " . __LINE__);
    try{
        if(!file_exists(BASE_PATH . "/Dinamica/seguridad/ahead.php")){
            throw new Exception("Adios mundo cruel... Fallo en el fundamento inherente a la seguridad. El archivo ".__FILE__." Viola el acceso al recurso");
        }
    } catch (Exception $ex) {
 ...
 */
        public function dinamica($ixEvalua){
            ...
            $analisis=array();
            #Logica de deriva de flujo a traves del fallo o del fluir normal
            if(!$ixEvalua["E"][0]){
                foreach($this->dinamica["fallos"] as $idVal => $valEnd){
                    unset($analisis);
                    $analisis=array();
                    echo $ixEvalua["C"][0]."<br>\n";
                    
                    $analisis[]=explode("_",$idVal);
                    $analisis[]=explode("_",$ixEvalua["C"][0]);
                    if($analisis[0][0]==$analisis[1][0]){
                        $ruta="/Dinamica/fallos/".$idVal."/".$ixEvalua["C"][0]."/index.php";
                    }
                }
                //echo $ruta . "<br>\n";
                echo "<pre>";
                print_r($ixEvalua);
                print_r($this->dinamica);
                echo "</pre>";
                exit;
    ...
    $modoRtm->registroMod("Hola mundo en Salida en dinamica ". __FILE__ . " => " . __LINE__);